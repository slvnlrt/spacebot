...
type SectionId = "soul" | "identity" | "user" | "routing" | "tuning" | "compaction" | "cortex" | "coalesce" | "memory" | "memory_injection" | "browser" | "sandbox";
...
export function AgentConfig({ agentId }: AgentConfigProps) {
	...
	useEffect(() => {
		if (search.tab) {
			const validSections: SectionId[] = ["soul", "identity", "user", "routing", "tuning", "compaction", "cortex", "coalesce", "memory", "memory_injection", "browser", "sandbox"];
	...
	const [localValues, setLocalValues] = useState<ConfigValues>(() => {
		...
		switch (sectionId) {
			...
			case "memory_injection":
				return { ...config.memory_injection } as ConfigValues;
			case "browser":
