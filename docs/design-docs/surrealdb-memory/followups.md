...
| 14 | 🔵 | **SQLite construction branch duplicated 4×** across the cfg-split. | **RESOLVED (Plan E)** — extracted `crate::memory::sqlite_backend_arc(store, &lance, agent_id)`; the 4 arms collapse to one call each (site-specific `MemoryStore` constructor preserved). −20 lines, behaviour-preserving. |
