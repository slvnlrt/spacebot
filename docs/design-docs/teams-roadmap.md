...
| **Onboarding facilitation** | "Download Teams app package" button in the Channels UI — **SHIPPED** in `pr/teams-channels-ui` (PR #608): `GET /api/messaging/teams/app-package?app_id=` builds a correct manifest (1.17, distinct persistent app id, Spacebot icons) + zips it; doc `teams-setup.mdx` Step 3. Live-tested 2026-06-29 (manifest/icons/persistence/400 all correct). A `spacebot teams-manifest` CLI equivalent remains optional. | partly shipped |
| **`instance_dir` startup race** (found 2026-06-29 live-testing the app-package endpoint) | NOT Teams-specific. The HTTP server starts listening ~1.2s **before** `set_instance_dir` runs, so any request in that cold-start window sees `instance_dir = PathBuf::new()` (empty). Observed: the first `app-package` download after a daemon (re)start wrote its `teams_manifest_id.json` sidecar to the process **cwd (`/`)** and returned a non-persisted id (the persistence guarantee breaks for that first call). Affects **every** `instance_dir.load()` consumer, not just this endpoint. **Root-cause fix (preferred, app-wide):** order startup so `set_instance_dir` completes **before** the axum server accepts connections (vs. a per-handler 503 "starting" band-aid). Narrow window (a human can't load+type+click in 1.2s) but a real correctness bug with a filesystem-pollution symptom. | backend hardening |
