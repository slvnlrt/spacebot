...
async fn save_load_roundtrip() {
    ...
    store
        .save(&m, Some(&emb(0.1, 0.2, 0.3, 0.4)))
        .await
        .unwrap();
...
async fn forget_excludes_from_reads() {
    ...
    store
        .save(&m, Some(&emb(0.9, 0.9, 0.9, 0.9)))
        .await
        .unwrap();
    ...
    let v = store
        .vector_search(&emb(0.9, 0.9, 0.9, 0.9), 5)
        .await
        .unwrap();
...
async fn associations_both_directions_and_neighbors() {
    ...
    store
        .create_association(
            &Association::new(&a.id, &b.id, RelationType::RelatedTo).with_weight(0.8),
        )
        .await
        .unwrap();
    store
    ...
    for i in 0..20 {
        ...
        store
            .save(
                ...
                Some(&emb(f, 1.0 - f, 0.5, 0.5)),
            )
            .await
            .unwrap();
    }
    let v = store
        .vector_search(&emb(0.5, 0.5, 0.5, 0.5), 5)
        .await
        .unwrap();
    assert_eq!(v.len(), 5);
...
async fn merge_rewires_and_soft_deletes() {
    ...
    store
        .save(&s, Some(&emb(0.5, 0.5, 0.5, 0.5)))
        .await
        .unwrap();
    store
        .save(&l, Some(&emb(0.5, 0.5, 0.5, 0.51)))
        .await
        .unwrap();
    store
        .save(&x, Some(&emb(0.1, 0.2, 0.3, 0.4)))
        .await
        .unwrap();
    store
    ...
    store
        .merge(
            &s.id,
            &l.id,
            "survivor saturn\n\nloser saturn rockets",
            Some(&emb(0.5, 0.5, 0.5, 0.5)),
        )
        .await
        .unwrap();
    ...
    assert!(
        s_assocs
            .iter()
            .any(|a| a.target_id == x.id || a.source_id == x.id)
    );
    assert!(
        s_assocs
            .iter()
