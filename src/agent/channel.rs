...
pub(crate) fn is_injection_block(message: &rig::message::Message) -> bool {
    match message {
        rig::message::Message::User { content } => content
            .iter()
            .any(|item| matches!(item, UserContent::Text(t) if t.text.starts_with(INJECTION_BLOCK_PREFIX))),
        _ => false,
    }
}
...
pub struct Channel {
    ...
    message_count: usize,
    /// Monotonic per-channel turn counter for re-injection delay.
    current_turn: usize,
    /// Per-channel state for memory re-injection and semantic dedup.
...
impl Channel {
    ...
    async fn handle_message_batch(&mut self, messages: Vec<InboundMessage>) -> Result<()> {
        ...
        // Pre-hook: compute memory injection based on the batched text.
    ...
    async fn handle_message(&mut self, message: InboundMessage) -> Result<()> {
        ...
        // Pre-hook: compute memory injection for regular user turns only.
    ...
    /// Compute pre-turn memory context to inject ahead of the user message.
    async fn compute_memory_injection(&mut self, user_text: &str) -> Option<String> {
        ...
        let mut lines = vec!["[Relevant to this message]".to_string()];
    ...
    #[tracing::instrument(skip(self, user_text, injected_context, system_prompt, attachment_content), fields(channel_id = %self.id, agent_id = %self.deps.agent_id))]
    async fn run_agent_turn(
        ...
        if let Some(context_text) = injected_context {
            let mut history = self.state.history.write().await;
            ...
            prune_old_injection_blocks(&mut history, max_keep);

            let injected_message = format!("{INJECTION_BLOCK_PREFIX}\n{}", context_text);
            history.push(rig::message::Message::User {
                content: OneOrMany::one(UserContent::text(injected_message)),
            });
            drop(history);
        } else {
            let mut history = self.state.history.write().await;
